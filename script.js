document.addEventListener('DOMContentLoaded', () => {
            ...
            tracks: {
                ...
                social: {
                    ...
                    subjects: {
                        ...
                        '歷史':       { w_f: 0.40, w_e1: 0.30, w_e2: 0.00, w_e3: 0.30, note: '採用(不含1,2班)規則' },
                        '地理':       { w_f: 0.60, w_e1: 0.40, w_e2: 0.00, w_e3: 0.00 },
    ...
    function setUIState(masterScheme) {
        ...
        if (!fenghsinSelected) {
             renderScoreInputs(masterScheme, weights);
             elements.calculatorBody.classList.remove('hidden');
        } else {
            const subjectSelected = !!weights;
    ...
    function initialize() {
            ...
            if (savedGrade) {
                ...
                if (savedTrack) {
                    ...
                    if (savedSubject) { 
                        ...
                        handleSubjectChange(); // Trigger this to show class input if needed
                    }
                }
            }
        }
        ...
        if (elements.classInput && savedClassNum) {
            elements.classInput.value = savedClassNum;
        }
        loadAllScores();
        updateCalculationModeUI();
    }
    ...
    elements.classInput?.addEventListener('input', () => { localStorage.setItem(LS_KEYS.class, elements.classInput.value); setUIState('fenghsin'); displaySelectedWeights(); });
