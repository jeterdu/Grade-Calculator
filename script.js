document.addEventListener('DOMContentLoaded', () => {
    console.log("腳本開始執行: DOMContentLoaded 事件已觸發");
    ...
    function updateScoreInputUI(masterScheme) {
        console.log(`正在根據模式 "${masterScheme}" 更新平時成績輸入框...`);
        ...
        if (!p1_label || !p2_container || !p3_container) {
            console.error("偵錯：找不到一個或多個平時成績的元件。");
            alert("偵錯：找不到平時成績的元件，無法切換輸入框數量。");
            return;
        }
        ...
        console.log("平時成績輸入框更新完畢。");
    }

    function setUIState(masterScheme) {
        console.log(`正在設定主 UI 狀態為 "${masterScheme}"...`);
    ...
    function updateCalculationModeUI() {
        console.log("正在更新計算模式 UI (期末考欄位顯示狀態)...");
        // alert("正在更新期末考欄位顯示狀態..."); // Aggressive debugging
        try {
            ...
            if (!customTargetContainer || !finalExamItem) {
                console.error("UI 更新錯誤：找不到 'customTargetInputContainer' 或 's_e3_item'。");
                alert("UI更新時發生致命錯誤：找不到期末考成績的容器(div)。請確認 index.html 是最新版本。");
                return;
            }
            ...
            console.log("期末考欄位顯示狀態更新完畢。");
        } catch(e) {
            console.error("updateCalculationModeUI 函數執行失敗:", e);
            alert("切換計算模式時發生內部錯誤，請按 F12 打開 Console 查看詳情。");
        }
    }

    // --- EVENT HANDLERS ---
    console.log("正在附加事件監聽器...");
    elements.masterSchemeRadios.forEach(radio => { radio.addEventListener('change', (e) => { console.log("主計分體系改變！"); const selectedScheme = e.target.value; localStorage.setItem(LS_KEYS.masterScheme, selectedScheme); setUIState(selectedScheme); }); });
    elements.gradeSelect.addEventListener('change', (e) => { console.log("年級已改變！"); const gradeKey = e.target.value; populateTracks(gradeKey); elements.trackSelect.value = ''; elements.subjectSelect.innerHTML = '<option value="">請先選擇組別</option>'; elements.subjectSelect.disabled = true; localStorage.setItem(LS_KEYS.grade, gradeKey); localStorage.removeItem(LS_KEYS.track); localStorage.removeItem(LS_KEYS.subject); setUIState('fenghsin'); });
    elements.trackSelect.addEventListener('change', (e) => { console.log("組別已改變！"); const gradeKey = elements.gradeSelect.value; const trackKey = e.target.value; populateSubjects(gradeKey, trackKey); elements.subjectSelect.value = ''; localStorage.setItem(LS_KEYS.track, trackKey); localStorage.removeItem(LS_KEYS.subject); setUIState('fenghsin'); });
    elements.subjectSelect.addEventListener('change', (e) => { console.log("科目已改變！"); const subject = e.target.value; localStorage.setItem(LS_KEYS.subject, subject); setUIState('fenghsin'); displaySelectedWeights(); });
    
    elements.calculationModeRadios.forEach(radio => {
        radio.addEventListener('change', (e) => {
            alert("計算模式已改變！"); // Add alert for direct feedback
            console.log("計算模式已改變！");
    ...
    // --- CALCULATION LOGIC & OTHER FUNCTIONS ---
    // The rest of the functions (getWeights, displaySelectedWeights, calculateAndDisplay, etc.) remain the same.
    // I am including the full script for completeness.
    function getWeights() { const masterScheme = getSelectedRadioValue('masterScheme'); switch (masterScheme) { case 'generic64': return { exam: 0.6, formative: 0.4, label: '通用方式 (60/40)' }; case 'generic73': return { exam: 0.7, formative: 0.3, label: '通用方式 (70/30)' }; case 'fenghsin': const grade = elements.gradeSelect.value; const track = elements.trackSelect.value; const subject = elements.subjectSelect.value; if (!grade || !track || !subject) return null; return FENG_HSIN_RULES[grade]?.tracks[track]?.subjects[subject] || null; default: return null; } }
    ...
    function calculateAndDisplay() {
        alert("計算按鈕已點擊！"); // Add alert for direct feedback
        console.log("計算按鈕已點擊！");
        try {
        ...
        } catch(e) { console.error("計算時發生錯誤:", e); alert("計算時發生預期外的錯誤，請按 F12 打開 Console 查看詳情。"); }
    }
    ...
    // Final check for event listeners
    ...
    // Populate Functions Definition (moved here for clarity, they are defined before use)
